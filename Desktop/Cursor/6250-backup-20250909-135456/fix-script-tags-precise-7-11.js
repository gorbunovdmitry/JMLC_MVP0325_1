...
// Исправляем лишние закрывающие script теги в вариантах 7-11 более точно
...
for (let i = 7; i <= 11; i++) {
    ...
    if (fs.existsSync(designerPath)) {
        ...
        // Оставляем только те, которые соответствуют открывающим (не самозакрывающимся)
        ...
        for (let line of lines) {
            if (line.includes('<script') && !line.includes('</script>')) {
                // Открывающий script тег (не самозакрывающийся)
                scriptCount++;
                newLines.push(line);
