...
// Добавляем функцию getStickerClassificationFromSelections в варианты 8-9

for (let i = 8; i <= 9; i++) {
    console.log(`Добавляем функцию классификации в 6250-${i}...`);
    
    const designerPath = `6250-${i}/designer-advanced.html`;
    if (fs.existsSync(designerPath)) {
        let content = fs.readFileSync(designerPath, 'utf8');
        
        // Добавляем функцию после updateStickerPreview
        const insertPoint = '        }\n\n        // Sticker classification system';
        const classificationFunction = `        // Get sticker classification from current selections
...
`;
        ...
        content = content.replace(insertPoint, insertPoint + classificationFunction);
        
        fs.writeFileSync(designerPath, content);
        console.log(`✅ 6250-${i}/designer-advanced.html: функция классификации добавлена`);
    }
}

console.log('Готово! Функция классификации добавлена в варианты 8-9.');
